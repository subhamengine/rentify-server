...
app.post("/create",verifyToken, async (req, res) => {
  try {
    const { propertyName, place, area, numberOfBedrooms, details, seller_id, seller_name,picturePath } = req.body;
    const newProperty = await pool.query(
      "INSERT INTO \"rentifyProperties2\" (propertyName, place, area, numberOfBedrooms, details, seller_id, seller_name,picturePath) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *",
...
app.get("/get",verifyToken, async (req, res) => {
  try {
    const allProperties = await pool.query("SELECT * FROM \"rentifyProperties2\"");
...
app.get("/get/:id",verifyToken, async (req, res) => {
  try {
    const { id } = req.params;
    const user = await pool.query('SELECT * FROM \"rentifyProperties2\" WHERE seller_id = $1', [id]);
...
app.delete("/delete/:id",verifyToken, async (req, res) => {
  try {
    ...
    const deleteUser = await pool.query(
      'DELETE FROM \"rentifyProperties2\" WHERE id = $1', [id]
    );
